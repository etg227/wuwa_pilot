...
- 可选切人视觉同步，确认角色 UI 后再继续时间轴，减少等待造成的累计偏移。
- 实时显示当前、平均和最大时间偏差，方便实机校准节奏。
...
目前兼容 WWCOMBO v1～v3，并兼容早期社区文件中的 `MouseRightHoid` 历史拼写。当前视觉同步点主要用于切人动作，其他技能和动画仍按轴时间执行。
